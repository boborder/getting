...
function getLocalD1Db() {
  ...
  if (!pathToDb) {
    console.error('⚠️ Local D1 database not found. Try running `npm run db:touch` to create one.')
    process.exit(1)
  }
 ...
 */
export function createProductionD1Connection(accountId: string, databaseId: string, apiToken: string) {
  /**
  ...
  const queryClient: AsyncRemoteCallback = async (sql, params, method) => {
    return executeCloudflareD1Query(accountId, databaseId, apiToken, sql, params, method)
  }
  ...
  const batchQueryClient: AsyncBatchRemoteCallback = async (queries) => {
    ...
    for (const query of queries) {
      ...
      const result = await executeCloudflareD1Query(accountId, databaseId, apiToken, sql, params, method)
      results.push(result)
    }
